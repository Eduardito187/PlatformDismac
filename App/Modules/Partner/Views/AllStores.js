...
const AllSotres = ({ route, navigation }) => {
    ...
    const getListStore = async (token) => {
        try {
            const response = await axios.get(URL_API('partner/getStoresList'), GET_HEADER_TOKEN(token));
                            ...
                            <View key={generateCustomId()} style={contentScrollItem}>
                                <View style={ItemContainerScrol}>
                                    ...
                                    <View style={[ItemCountScroll, ALING_CENTER]}>
                                        <Text style={SubTitleText}>{state.count_warehouse} {state.count_warehouse > 1 ? 'warehouses' : 'warehouse'}</Text>
                                    </View>
