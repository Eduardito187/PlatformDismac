import { URL_API,GET_HEADERS,CREATE_BODY_SEARCH_ACCOUN,GET_HEADER_ACCOUNT } from './API';
...
export async function searchAccountAPI(Search,EndAction) {
    axios.post(URL_API("search/account"),CREATE_BODY_SEARCH_ACCOUN(Search),GET_HEADER_ACCOUNT()).then(res => {
        if(res.data != null){
            EndAction(res.data.response, res.data.responseText);
        }else{
            EndAction(false, "Algo salio mal.");
